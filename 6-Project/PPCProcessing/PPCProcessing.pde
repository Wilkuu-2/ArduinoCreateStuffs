 ...
 * 
 *  This code has been rewrittien in about 4 hours, which i find commendable
 *
...
// -- Calls the WaveSet to save the file

final static String filename = "wavefile.txt";

void saveWaveSet(){
  PrintWriter file = createWriter(filename);
  wSetTot.writeToFile(file);
  file.close();
}
